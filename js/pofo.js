(($)=>{
  ...
  class pofo {
      ...
      section4(){
       ...
       let n = 8;                    //이미지 갤러리 보이는 갯수
       let cols = 4;                 //칸수 기본 4칸
       let rows = Math.ceil(n/cols);                 //줄수 기본 2줄 = Math.ceil 자리올림(n/cols) Math.floor 자리내림
       
       let winW = $(window).width(); //창너비 100vw
        ...
        let setId = []; // 타이머 정지 변수
            ...
            function svgAniFn(){
              $.each(svgCircle, function(idx,obj){
                  ...
                  //3. 각 요소(원형)의 길이를 10초간의 백분율을 구현하기위해 작은 조각단위로 조각단위의 길이를 구한다.
                  //   1초 동안에 원의 작은 조각단위(piece, chip)의 길이
                  piece[idx] = svgArr[idx] / second / 100;
    
                  //4. 퍼센트 해당하는 길이를 구한다.
                  perSize[idx] = svgArr[idx] * percent[idx];
    
                  //5. 조각단위를 누적하는 함수를 만든다.
                  function sumfn(){
